...
const FIELD_IDS = [
  ...
  'creditLimit', 'paymentTermsDays', 'currency', 'isActive',
];
...
function defaultForm(): CustomerForm {
  return {
    ...
    billingAddress: '', shippingAddress: '',
    creditLimit: '0.00', paymentTermsDays: '30',
    currency: 'SAR', isActive: 'true',
  };
}
...
export default function CustomerFormPage() {
  ...
  useEffect(() => {
    if (existing?.data) {
      ...
      setForm({
        ...
        creditLimit: String((Number(d['creditLimit'] ?? 0) / 100).toFixed(2)),
  ...
  const vatStatus = form.taxId.trim()
    ? isB2B ? 'B2B — Standard (Clearance)' : 'VAT format invalid (15 digits, starts with 3)'
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      isNew
        ? api.post('/customers', buildPayload()).then(r => r.data)
        : api.put(`/customers/${id}`, buildPayload()).then(r => r.data),
    onSuccess: (data: { data?: { id?: unknown } }) => {
      setStatusMsg('Saved ✓');
      setStatusType('success');
      qc.invalidateQueries({ queryKey: ['customers'] });
      const newId = data?.data?.id;
      if (leaveAfterSave.current) { leaveAfterSave.current = false; navigate('/customers'); return; }
      setTimeout(() => { setStatusMsg(''); if (isNew && newId) navigate(`/customers/${newId}`); }, 2000);
    },
    onError: (err: unknown) => {
  ...
  const f = (key: keyof CustomerForm, val: string) => setForm(prev => ({ ...prev, [key]: val }));
    ...
    <div style={{ maxWidth: 860, fontFamily: 'var(--font)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-muted)', margin: '4px 0 0' }}>
            {isNew ? 'Create a new customer record' : `Editing ${form.companyName}`}
          </p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => navigate('/customers')} style={btnSecondary}>Cancel</button>
      ...
      <div style={card}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 32px' }}>
          ...
          <div>
            <KbdField id="creditLimit" label="Credit Limit (SAR)" type="number" value={form.creditLimit}
              onChange={v => f('creditLimit', v)} onNext={() => nav.nextField('creditLimit')}
              onPrev={() => nav.prevField('creditLimit')} />
            <KbdField id="paymentTermsDays" label="Payment Terms (days)" type="number" value={form.paymentTermsDays}
              onChange={v => f('paymentTermsDays', v)} onNext={() => nav.nextField('paymentTermsDays')}
              onPrev={() => nav.prevField('paymentTermsDays')} />
      ...
      <div style={card}>
        <p style={sectionLabel}>ZATCA Structured Address <span style={{ fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(required for B2B standard invoices)</span></p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 32px' }}>
          <div>
      ...
      {/* ── Legacy Addresses ── */}
      <div style={card}>
        <p style={sectionLabel}>Additional Addresses</p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 32px' }}>
          <KbdField id="billingAddress" label="Billing Address (free text)"
            value={form.billingAddress} onChange={v => f('billingAddress', v)}
            onNext={() => nav.nextField('billingAddress')} onPrev={() => nav.prevField('billingAddress')}
            multiline />
          <KbdField id="shippingAddress" label="Shipping Address"
            value={form.shippingAddress} onChange={v => f('shippingAddress', v)}
            onNext={() => nav.nextField('shippingAddress')} onPrev={() => nav.prevField('shippingAddress')}
            multiline />
        </div>
      </div>
