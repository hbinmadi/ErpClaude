...
export default function ArAgeingPage() {
  ...
  const { data, isLoading, error } = useQuery<{ data: ARRow[] }>({
    queryKey: ['ar-ageing', queryDate],
    queryFn: () =>
      api.get('/reports/ar-ageing', { params: { as_of_date: queryDate } }).then(r => r.data),
  });
  ...
  return (
    <div>
      <PageHeader title="AR Ageing" subtitle="Accounts receivable ageing report" />
            ...
            <table style={{ borderCollapse: 'collapse', fontSize: 13, width: '100%', fontFamily: 'var(--font)' }}>
              <thead>
                <tr style={{ background: 'var(--bg-elevated)' }}>
                  <th style={{ padding: '10px 14px', textAlign: 'left', color: 'var(--text-muted)', fontWeight: 600, fontSize: 11, textTransform: 'uppercase', letterSpacing: 0.6, borderBottom: '1px solid var(--border)' }}>Customer</th>
              ...
              <tbody>
                {rows.length === 0 ? (
                  ...
                  rows.map((row, i) => (
                    ...
                    >
                      <td style={{ padding: '10px 14px', color: 'var(--text)', borderBottom: '1px solid var(--border-muted)' }}>{row.customerName}</td>
